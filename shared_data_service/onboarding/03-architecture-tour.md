...
flowchart TB
    subgraph API edge
        ...
        M --> R[ModuleRoutes<br/>app/modules/shared/routes.py]
    end
    ...
    R --> BUS[VersionedModuleService<br/>app/modules/shared/service.py]
    B2 --> BUS
...
Since the spec-registry refactor there is no per-module service, repository,
or handler class: one generic `VersionedModuleService` and one generic
`VersionedRepository` run every module, each configured by that module's
`ModuleSpec` (declared at the bottom of its module file, e.g.
...
| Rule | Enforced where |
|---|---|
| API routes → service → repository → database; each layer sees only the next | `ModuleRoutes`' logic methods (`app/modules/shared/routes.py`) call `VersionedModuleService`, never a repository |
...
| Modules never touch another module's internals | modules communicate through events only; modules import from `modules/shared/` only, never from a sibling |
...
!!! note "Why the service layer is framework-free"
    `VersionedModuleService` is called by both edges. If it imported FastAPI
...
`POST /users`, from TCP to response. (`user` is a **scoped** module, so it
is reachable two ways: the top-level `/users` used here, and the
project-nested `POST /{project_id}/user` which additionally sets the user's
`project_id` from the path — see [Adding a Module](05-adding-a-module.md#nested-scoped-routing).
The pipeline below is identical; the scoped route just carries a scope.)
...
2. **Route translates.** The `create_user` route is generated by the shared
   `ModuleRoutes` (`app/modules/shared/routes.py`) from `USER_SPEC` — the
   module file has no route code. Its `create` logic method validates the
   strict `UserCreate` schema, fills in `uuid4()` if the client sent no id,
   builds a `UserData` from it, and calls the service's `create`. `UserCreate`
   is
...
4. **Idempotent insert.** `repo.insert_if_absent(User(...))` either inserts
   ...
   stored row is returned with HTTP 200 (the `create` logic method downgrades
   the default 201) — and the service **re-announces** the stored
...
sequenceDiagram
    ...
    participant SV as VersionedModuleService.apply_state_events
...
It loops `ALL_SPECS` and wires the *same* generic service twice per module
...
!!! note "Why not an `if consumer:` flag?"
    ...
    could do it, so no review needs to guard it. `VersionedModuleService`
...
`build_module_consumer(spec, ...)` call per registered spec
...
app/
  ...
  modules/
    __init__.py the ENTITY REGISTRY: ALL_SPECS — the only place a new module
    ...
    shared/     cross-module vocabulary AND machinery: errors.py (DomainError
                ...
                (paging/sort), filters.py (the Django-style field__op filter
                engine), spec.py (ModuleSpec + the q() column tags),
                repository.py (generic VersionedRepository — idempotent
                insert, version-guarded upsert, whitelisted+filtered list;
                whitelists derived from the model's q() tags), service.py
                (VersionedModuleService — the whole create/update/apply-events
    ...
    user.py     one complete module in ONE file: ORM model (incl. project_id
                scope column), UserData (floor — business model AND event
                payload), strict Create/Update, Out/PageOut/Filters/ListParams,
                USER_SPEC — ends at the spec, routes generated by ModuleRoutes;
                scoped under project (/{project_id}/user) plus a top-level
                /users. The template for every module
    project.py  the second module — a root (flat /project), same single-file shape
main.py         mode switch: api/both → uvicorn, consumer → asyncio runner
```

The rule of thumb: `api/`, `messaging/`, `database/`, `logging/` are
*infrastructure shared by every module*; `modules/<module>.py` is
*everything about one module*; `bootstrap/` loops the registry. Adding an
module means one new module file, one line in `ALL_SPECS`, one fixtures
