...
flowchart TB
    subgraph API edge
        ...
        M --> R[EntityRoutes<br/>app/modules/shared/routes.py]
    end
...
| Rule | Enforced where |
|---|---|
| API routes → service → repository → database; each layer sees only the next | `EntityRoutes`' logic methods (`app/modules/shared/routes.py`) call `VersionedEntityService`, never a repository |
...
2. **Route translates.** The `create_user` route is generated by the shared
   `EntityRoutes` (`app/modules/shared/routes.py`) from `USER_SPEC` — the
   entity file has no route code. Its `create` logic method validates the
...
app/
  ...
  modules/
    ...
    shared/     cross-module vocabulary AND machinery: errors.py (DomainError
                ...
                (EntityRoutes — generates the four CRUD routes for any spec),
    ...
    user.py     one complete entity in ONE file: ORM model, UserData (floor —
                ...
                routes generated by EntityRoutes; the template for every entity
