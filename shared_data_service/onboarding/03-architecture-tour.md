...
`POST /user`, from TCP to response:
...
app/
  ...
  modules/
    ...
    shared/     cross-module vocabulary AND machinery: errors.py (DomainError
                ...
                (paging/sort/filter), spec.py (ModuleSpec + the q() column
    ...
    user.py     one complete module in ONE file: ORM model, UserData (floor —
                business model AND event payload), strict Create/Update,
                Out/PageOut/Filters/ListParams, USER_SPEC — ends at the spec,
                routes generated by ModuleRoutes; the template for every module
    project.py  the second module, same single-file shape
